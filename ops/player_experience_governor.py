...
def main() -> int:
    ...
    reward_tempo = round(clamp((return_reward_avg / 110.0) + (quality_score / 20.0), 0.0, 1.0), 2)
    social_stickiness = round(clamp((social_pressure / 24.0) + (event_join_avg / 2200.0) + (social_loop_density / 5.0), 0.0, 1.0), 2)
    replay_pull = round(clamp((event_join_avg / 1800.0) + (retention_proxy / 3.5) + (replayable_loop_score / 5.0), 0.0, 1.0), 2)
