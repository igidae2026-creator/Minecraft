...
for server in lobby rpg_world dungeons boss_world events; do
  (
    cd "$ROOT_DIR/$server"
    nohup "$JAVA_BIN" $BACKEND_JAVA_OPTS -jar "$PAPER_JAR_NAME" nogui > "$ROOT_DIR/logs/${server}.log" 2>&1 &
...
(
  cd "$ROOT_DIR/proxy"
  nohup "$JAVA_BIN" $PROXY_JAVA_OPTS -jar "$VELOCITY_JAR" > "$ROOT_DIR/logs/velocity.log" 2>&1 &
